/*
**********QUERY KTH SMALLEST TRIMMMED NUMBER**********
...
int *smallestTrimmedNumbers(char **nums, int numsSize, int **queries, int queriesSize, int *queriesColSize, int *returnSize)
{
    *returnSize = queriesSize;
    ...
    int *qIndices = malloc(queriesSize * 4);
    for (int i = 0; i < queriesSize; i++)
        qIndices[i] = i;
    loadList = queries;
    qsort(qIndices, queriesSize, 4, cmp);
    char len = strlen(*nums);
    char digit = len - 1, qInd = 0, LSD[10][100], LSDlens[10], nIndices[numsSize];
    for (char i = 0; i < numsSize; i++)
        nIndices[i] = i;
    do
    {
        memset(LSDlens, 0, 10);
